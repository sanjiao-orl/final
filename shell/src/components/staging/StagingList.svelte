...
<div class="list">
  ...
  {#if candidates.generating}<div class="generating">● AI 生成中…{candidates.generating.text.length} 字</div>{/if}
...
<style>
  ...
  .generating { padding: 10px 12px; color: var(--accent); font-size: 11px; animation: pulse 1.5s ease-in-out infinite; } .empty { padding: 28px 14px; color: var(--muted); font-size: 12px; text-align: center; } @keyframes pulse { 50% { opacity: .55; } }
</style>