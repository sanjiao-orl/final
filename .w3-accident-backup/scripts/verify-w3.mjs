...
// 覆盖：/rewrite SSE 改写、/candidates 建-列-改、/tools 代理、/chat scope 归属 + /sessions?scope 过滤。
...
// ---- 1. /tools 代理：list_structure ----
const structRes = await fetch(`${base}/tools/list_structure`, {
  method: 'POST', headers: auth,
  body: JSON.stringify({ workDir: workDir.replaceAll('\\', '/') }),
});
if (!structRes.ok) fail('/tools/list_structure ' + structRes.status + ' ' + (await structRes.text()));
...
console.log(`[A] /tools 代理 OK：${chapter.relPath}，${chapter.wordCount} 字，场=${chapter.scenes?.length}`);
...
if (!rwRes.ok) fail('/rewrite ' + rwRes.status);
const rwText = await rwRes.text();
const doneMatch = /event: done\ndata: (\{.*\})/.exec(rwText);
if (!doneMatch) fail('/rewrite 无 done: ' + rwText.slice(-300));
...
console.log(`[A] /rewrite OK：text-delta=${deltas}，改写="${rewritten.slice(0, 60)}…"`);

// ---- 3. /candidates 建-列-改 ----
const cRes = await fetch(`${base}/candidates`, {
  method: 'POST', headers: auth,
  body: JSON.stringify({ chapter: chapter.relPath, original, proposed: rewritten, instruction: '加强画面感' }),
});
if (!cRes.ok) fail('POST /candidates ' + cRes.status);
const cand = (await cRes.json()).candidate;
const list1 = (await (await fetch(`${base}/candidates?status=pending`, { headers: auth })).json()).candidates;
if (!list1.some((c) => c.id === cand.id)) fail('pending 列表缺新建候选');
const pRes = await fetch(`${base}/candidates/${cand.id}`, {
...
const list2 = (await (await fetch(`${base}/candidates?status=pending`, { headers: auth })).json()).candidates;
if (list2.some((c) => c.id === cand.id)) fail('adopted 后仍在 pending 列表');
console.log('[A] /candidates 建-列-改 OK（pending→adopted 状态机正确）');

// ---- 4. /chat scope 归属 + /sessions?scope 过滤 ----
async function chat(scope) {
  const res = await fetch(`${base}/chat`, {
    method: 'POST', headers: auth,
    body: JSON.stringify({ text: '你觉得这枚铜钱可以埋什么伏笔？一句话。', scope, workDir: workDir.replaceAll('\\', '/') }),
  });
  if (!res.ok) fail('/chat ' + res.status);
  const t = await res.text();
  const m = /event: done\ndata: (\{.*\})/.exec(t);
  if (!m) fail('/chat 无 done');
  return JSON.parse(m[1]).sessionId;
}
await chat(''); // 无归属讨论
await chat(chapter.relPath); // 章节内讨论
const all = (await (await fetch(`${base}/sessions`, { headers: auth })).json()).sessions;
const none = (await (await fetch(`${base}/sessions?scope=`, { headers: auth })).json()).sessions;
const inCh = (await (await fetch(`${base}/sessions?scope=${encodeURIComponent(chapter.relPath)}`, { headers: auth })).json()).sessions;
