...
6. **缓做**:管理 UI(skill 数量管不过来再评估);壳内指令模板(SelectionPopover/整改模板)搬家,UI 耦合,单独小批。~~热重载(现靠重启 core 生效)~~——**已落地**(2026-08-16,随 0.2.2):prompt 按 mtime 感知热重载、skill 清单每次请求现扫,改文件即生效,不再靠重启 core。
