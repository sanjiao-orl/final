 ...
 * 编号规范：新产出/重编号一律阿拉伯数字（第1章·少年）；汉字旧数据照常解析（兼容）。
 */
...
describe('create_chapter', () => {
  it('编号接续：legacy 阿拉伯编号 第1章/第2章 时新建得 第3章（阿拉伯规范名）', () => {
    ...
    expect(res).toEqual({ ok: true, relPath: 'manuscript/第3章·风起.md' });
    expect(fs.existsSync(path.join(work, 'manuscript/第3章·风起.md'))).toBe(true);
  });

  it('汉字旧数据兼容：第一章 之后新建得 第2章（按数值接续、输出阿拉伯）', () => {
    const work = makeWorkDir();
    writeTree(work, {
      'manuscript/第一章·少年.md': chapter('第一章·少年'),
      'manuscript/第三章·客栈.md': chapter('第三章·客栈'),
    });
    const res = createChapter(work, undefined, '风起');
    expect(res).toEqual({ ok: true, relPath: 'manuscript/第4章·风起.md' });
    expect(fs.existsSync(path.join(work, 'manuscript/第4章·风起.md'))).toBe(true);
    // 旧汉字文件原样保留，未被动过
    expect(fs.existsSync(path.join(work, 'manuscript/第一章·少年.md'))).toBe(true);
    expect(fs.readFileSync(path.join(work, 'manuscript/第一章·少年.md'), 'utf8')).toBe(
      chapter('第一章·少年'),
    );
  });
  ...
  it('卷内编号接续，且根目录散章不影响卷内编号', () => {
    ...
    expect(createChapter(work, '第一卷', '风起').relPath).toBe('manuscript/第一卷/第4章·风起.md');
    // 根卷只认根下的章（第2章），与第一卷无关
    expect(createChapter(work).relPath).toBe('manuscript/第3章·新章.md');
  });
  ...
  it('模板含 title/status/id，goal 传了才写', () => {
    ...
    expect(parseFrontmatter(content1)).toMatchObject({ title: '第1章·少年', status: '草稿', id: expect.any(String) });
  ...
  it('拒绝带编号前缀的 title、带路径分隔符的 volume', () => {
    ...
    expect(() => createChapter(work, undefined, '第五章·少年')).toThrow(/编号前缀/);
  ...
  it('同名文件已存在抛错（不覆盖）', () => {
    ...
    writeTree(work, { 'manuscript/第1章·少年.md/占位.txt': 'x' });
    expect(() => createChapter(work, undefined, '少年')).toThrow(/已存在/);
    expect(fs.statSync(path.join(work, 'manuscript/第1章·少年.md')).isDirectory()).toBe(true);
  });
});
...
describe('create_volume', () => {
  it('编号接续（只认匹配「第N卷」模式的目录），默认标题 新卷', () => {
    const work = makeWorkDir();
    fs.mkdirSync(path.join(work, 'manuscript/第1卷·风起'), { recursive: true });
    expect(createVolume(work, '云涌')).toEqual({ ok: true, volumePath: 'manuscript/第2卷·云涌' });
    expect(createVolume(work)).toEqual({ ok: true, volumePath: 'manuscript/第3卷·新卷' });
    expect(fs.statSync(path.join(work, 'manuscript/第2卷·云涌')).isDirectory()).toBe(true);
  });

  it('汉字旧数据兼容：第一卷 之后新建得 第2卷（按数值接续、输出阿拉伯）', () => {
    const work = makeWorkDir();
    fs.mkdirSync(path.join(work, 'manuscript/第一卷·风起'), { recursive: true });
    expect(createVolume(work, '云涌')).toEqual({ ok: true, volumePath: 'manuscript/第2卷·云涌' });
    expect(fs.existsSync(path.join(work, 'manuscript/第一卷·风起'))).toBe(true);
  });

  it('拒绝带编号前缀的 title 与路径分隔符；同名卷目录已存在抛错', () => {
    const work = makeWorkDir();
    expect(() => createVolume(work, '第3卷·风起')).toThrow(/编号前缀/);
    expect(() => createVolume(work, '第三卷·风起')).toThrow(/编号前缀/);
    expect(() => createVolume(work, '风/起')).toThrow(/分隔符/);
    // 同名条目是文件而非目录：编号计数跳过，但存在性检查拦下（不覆盖）
    writeTree(work, { 'manuscript/第1卷·风起': '不是目录' });
...
describe('rename_chapter', () => {
  it('保留编号只换标题，同步 frontmatter title，其余字段与正文不动', () => {
    ...
    const content =
      '---\ntitle: 第2章·客栈\nstatus: 完稿\ncustom: 保留我\n---\n\n客栈正文。\n### 场景甲';
    writeTree(work, { 'manuscript/第一卷/第2章·客栈.md': content });
    const res = renameChapter(work, 'manuscript/第一卷/第2章·客栈.md', '少年');
    expect(res).toEqual({ ok: true, relPath: 'manuscript/第一卷/第2章·少年.md' });
    expect(fs.readFileSync(path.join(work, 'manuscript/第一卷/第2章·少年.md'), 'utf8')).toBe(
      '---\ntitle: 第2章·少年\nstatus: 完稿\ncustom: 保留我\n---\n\n客栈正文。\n### 场景甲',
    );
    expect(fs.existsSync(path.join(work, 'manuscript/第一卷/第2章·客栈.md'))).toBe(false);
  });

  it('汉字旧数据兼容：第一章·旧 改名输出 第1章·新（阿拉伯）', () => {
    const work = makeWorkDir();
    writeTree(work, { 'manuscript/第一章·旧.md': chapter('第一章·旧') });
    expect(renameChapter(work, 'manuscript/第一章·旧.md', '新').relPath).toBe(
      'manuscript/第1章·新.md',
    );
    expect(fs.readFileSync(path.join(work, 'manuscript/第1章·新.md'), 'utf8')).toBe(
      '---\ntitle: 第1章·新\nstatus: 草稿\n---\n\n正文内容。',
    );
  });
  ...
  it('无 frontmatter 的文件改名时补建 fm 块', () => {
    const work = makeWorkDir();
    writeTree(work, { 'manuscript/第1章·旧.md': '只有正文。' });
  ...
  it('目标同名已存在抛错；带编号前缀的 title 拒绝', () => {
    ...
    writeTree(work, {
      'manuscript/第1章·少年.md': chapter('第1章·少年'),
      'manuscript/第1章·客栈.md': chapter('第1章·客栈'),
    });
...
describe('rename_volume', () => {
  it('匹配卷名模式时保留编号只换标题；不匹配直接用新标题', () => {
    const work = makeWorkDir();
    fs.mkdirSync(path.join(work, 'manuscript/第2卷·客栈'), { recursive: true });
    expect(renameVolume(work, 'manuscript/第2卷·客栈', '风起')).toEqual({
      ok: true,
      volumePath: 'manuscript/第2卷·风起',
    });
    expect(fs.statSync(path.join(work, 'manuscript/第2卷·风起')).isDirectory()).toBe(true);

    // 汉字编号同样匹配模式 → 保留编号只换标题，编号按规范输出阿拉伯
  ...
  it('拒绝等于 manuscript 本身或非 manuscript/ 前缀；目标同名已存在抛错', () => {
    const work = makeWorkDir();
    fs.mkdirSync(path.join(work, 'manuscript/第1卷·风起'), { recursive: true });
    fs.mkdirSync(path.join(work, 'manuscript/第1卷·云涌'), { recursive: true });
    expect(() => renameVolume(work, 'manuscript', 'x')).toThrow();
    expect(() => renameVolume(work, 'notes', 'x')).toThrow();
    expect(() => renameVolume(work, 'manuscript/第1卷·风起', '云涌')).toThrow(/已存在/);
  });
});
...
describe('move_chapter', () => {
  it('重排后编号 1..N 连续、用户标题保留、fm title 同步', () => {
    ...
    writeTree(work, {
      'manuscript/第一卷/第1章·少年.md': chapter('第1章·少年'),
      'manuscript/第一卷/第2章·客栈.md': chapter('第2章·客栈'),
      'manuscript/第一卷/第3章·风起.md': chapter('第3章·风起'),
    });
    const res = moveChapter(work, 'manuscript/第一卷/第3章·风起.md', 0);
    expect(res.ok).toBe(true);
    expect(res.renumbered.map((c) => c.title)).toEqual(['第1章·风起', '第2章·少年', '第3章·客栈']);
    expect(res.renumbered.map((c) => c.relPath)).toEqual([
      'manuscript/第一卷/第1章·风起.md',
      'manuscript/第一卷/第2章·少年.md',
      'manuscript/第一卷/第3章·客栈.md',
    ]);
    expect(fs.readFileSync(path.join(work, 'manuscript/第一卷/第1章·风起.md'), 'utf8')).toContain(
      'title: 第1章·风起',
    );
    expect(fs.readFileSync(path.join(work, 'manuscript/第一卷/第3章·客栈.md'), 'utf8')).toContain(
      'title: 第3章·客栈',
    );
    expect(fs.existsSync(path.join(work, 'manuscript/第一卷/第3章·风起.md'))).toBe(false);
  });

  it('汉字旧数据兼容：重排旧汉字章输出阿拉伯编号', () => {
    const work = makeWorkDir();
    writeTree(work, {
  ...
  it('移到同位置幂等：两次调用结果一致且不再改名', () => {
    ...
    const once = moveChapter(work, 'manuscript/第一卷/第1章·少年.md', 0);
    expect(once.renumbered.map((c) => c.title)).toEqual(['第1章·少年', '第2章·客栈']);
    const twice = moveChapter(work, 'manuscript/第一卷/第1章·少年.md', 0);
  ...
  it('不匹配模式的旧章名不动，但排序仍按文件名', () => {
    ...
    const res = moveChapter(work, 'manuscript/第一卷/第1章·少年.md', 1);
    expect(res.renumbered.map((c) => c.title)).toEqual(['旧稿·未编号', '第2章·少年']);
  ...
  it('不支持跨卷移动（toIndex 越界抛错）', () => {
    ...
    expect(() => moveChapter(work, 'manuscript/第一卷/第1章·少年.md', 2)).toThrow(/跨卷/);
    expect(() => moveChapter(work, 'manuscript/第一卷/第1章·少年.md', -1)).toThrow();
    expect(() => moveChapter(work, 'manuscript/第一卷/没有.md', 0)).toThrow();
  });

  it('目标名冲突整体拒绝（不改任何文件）', () => {
    const work = makeWorkDir();
    const ch1 = chapter('第1章·少年');
    const ch2 = chapter('第2章·少年'); // 同用户标题：重排后目标名互相撞车
    writeTree(work, {
      'manuscript/第一卷/第1章·少年.md': ch1,
      'manuscript/第一卷/第2章·少年.md': ch2,
    });
    expect(() => moveChapter(work, 'manuscript/第一卷/第1章·少年.md', 1)).toThrow(/冲突/);
    expect(sortedNames(fs.readdirSync(path.join(work, 'manuscript/第一卷')))).toEqual([
      '第1章·少年.md',
      '第2章·少年.md',
    ]);
    expect(fs.readFileSync(path.join(work, 'manuscript/第一卷/第1章·少年.md'), 'utf8')).toBe(ch1);
    expect(fs.readFileSync(path.join(work, 'manuscript/第一卷/第2章·少年.md'), 'utf8')).toBe(ch2);
  });
});
...
describe('move_volume', () => {
  it('重排：卷名按最终顺序重编号 1..N', () => {
    const work = makeWorkDir();
    for (const v of ['第1卷·风起', '第2卷·云涌', '第3卷·雷动']) {
      fs.mkdirSync(path.join(work, 'manuscript', v), { recursive: true });
    }
    const res = moveVolume(work, 'manuscript/第3卷·雷动', 0);
    expect(res.ok).toBe(true);
    expect(res.renumbered.map((v) => v.title)).toEqual(['第1卷·雷动', '第2卷·风起', '第3卷·云涌']);
  ...
  it('未匹配卷名的目录不改名但参与排序', () => {
    ...
    const res = moveVolume(work, 'manuscript/第1卷·风起', 1);
    expect(res.renumbered.map((v) => v.title)).toEqual(['杂集', '第2卷·风起']);
    expect(sortedNames(fs.readdirSync(path.join(work, 'manuscript')))).toEqual(['杂集', '第2卷·风起']);
  });

  it('目标名冲突整体拒绝；toIndex 越界抛错', () => {
    const work = makeWorkDir();
    fs.mkdirSync(path.join(work, 'manuscript/第1卷·风起'), { recursive: true });
    fs.mkdirSync(path.join(work, 'manuscript/第2卷·风起'), { recursive: true });
